...
```bash
sagui agent "Open Safari and search for Swift"                    # Uses the on-device Apple model
sagui agent "Click the submit button" --provider openai            # Run with OpenAI
sagui agent "Fill in the form" --model gpt-5.6-sol --provider openai --max-iterations 10
sagui agent "Press the Save button" --vision-mode automatic       # Prefer semantic AX elements when available
```

| Required | Optional |
|---|---|
| `<goal>` (string) | `--api-key <key>` (or `OPENAI_API_KEY` env), `--model <model>` (default: `gpt-5.6-sol`), `--provider <on-device|pcc|openai>` (default: `on-device`), `--fallback-on-device` (opt-in), `--vision-mode <always|automatic|never>` (default: `always`), `--max-iterations <n>` (default: 20), `--delay <seconds>` (default: 1.0), `--no-screen-context` |

The command prints the selected provider at startup and a concise
